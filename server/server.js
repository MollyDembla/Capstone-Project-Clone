...
app.use('/datasets', express.static(path.join(PROJECT_ROOT, 'datasets')));
...
function buildPrompt(data) {
	...
	const heightMeters = Number(data.heightCm) > 0 ? Number(data.heightCm) / 100 : null;
	const bmi = Number.isFinite(heightMeters) && heightMeters > 0 && Number.isFinite(Number(data.weightKg))
		? Number(data.weightKg) / (heightMeters * heightMeters)
		: null;
	...
	return [
		...
		'- Add a short sedentary-lifestyle support layer (movement breaks, posture hygiene, sleep routine, hydration, and meal timing) as an additional recommendation, without changing the required section structure.',
		'',
		'Input data:',
		`- Asana: ${data.asana}`,
		...
		`- Age: ${data.age}`,
		`- Height (cm): ${data.heightCm ?? 'unknown'}`,
		`- Weight (kg): ${data.weightKg ?? 'unknown'}`,
		`- BMI: ${Number.isFinite(bmi) ? bmi.toFixed(2) : 'unknown'}`,
		`- Flexibility level: ${data.flexibility}`,
		`- Experience level: ${data.experience}`,
		`- Activity level: ${data.activityLevel ?? 'unknown'}`,
		`- Eating habits: ${data.eatingHabits ?? 'unknown'}`,
		`- Sleep hours/day: ${Number.isFinite(Number(data.sleepHours)) ? Number(data.sleepHours).toFixed(1) : 'unknown'}`,
...
function fallbackReport(data) {
	...
	const bmi = Number.isFinite(heightCm) && Number.isFinite(weightKg) && heightCm > 0
	...
	return [
		...
		`- Profile: age=${data.age ?? 'N/A'}, height=${Number.isFinite(heightCm) ? `${heightCm}cm` : 'N/A'}, weight=${Number.isFinite(weightKg) ? `${weightKg}kg` : 'N/A'}, BMI=${Number.isFinite(bmi) ? bmi.toFixed(1) : 'N/A'}`,
		`- Lifestyle context: activity=${data.activityLevel || 'N/A'}, eating=${data.eatingHabits || 'N/A'}, sleep=${Number.isFinite(Number(data.sleepHours)) ? `${Number(data.sleepHours).toFixed(1)}h` : 'N/A'}, healthNotes=${data.healthNotes || 'none'}`,
		...
		`- Sedentary reset target: add 3-5 movement breaks/day based on activity level (${data.activityLevel || 'N/A'}).`,
		`- Recovery support: build sleep consistency around ${Number.isFinite(Number(data.sleepHours)) ? `${Number(data.sleepHours).toFixed(1)}h/night` : 'regular bedtime'} and hydration reminders.`,
		...(improvements.length
		...
		`- Lifestyle support add-on: for sedentary recovery, include daily movement breaks, posture resets, hydration consistency, and sleep regularity (activity=${data.activityLevel || 'N/A'}, eating=${data.eatingHabits || 'N/A'}, sleep=${Number.isFinite(Number(data.sleepHours)) ? `${Number(data.sleepHours).toFixed(1)}h` : 'N/A'}).`,
	].join('\n');
}
...
function buildChatPrompt(question, context) {
	...
	return [
		...
		`- User profile: age=${profile?.age ?? 'unknown'}, heightCm=${profile?.heightCm ?? 'unknown'}, weightKg=${profile?.weightKg ?? 'unknown'}, flexibility=${profile?.flexibility ?? 'unknown'}, experience=${profile?.experience ?? 'unknown'}, activityLevel=${profile?.activityLevel ?? 'unknown'}, eatingHabits=${profile?.eatingHabits ?? 'unknown'}, sleepHours=${Number.isFinite(Number(profile?.sleepHours)) ? Number(profile.sleepHours).toFixed(1) : 'unknown'}, healthNotes=${profile?.healthNotes || 'none'}`,
...
app.use((_req, res) => {
	res.sendFile(path.join(HAS_DIST_BUILD ? DIST_DIR : PROJECT_ROOT, 'index.html'));
});
